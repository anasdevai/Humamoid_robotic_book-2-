...
if env_path.exists():
    ...
    load_dotenv()  # Will use Vercel env vars or local .env in current dir
...
class RAGRetriever:
    def __init__(self):
        ...
        # Default collection name - updated for Cohere
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        """
        try:
            # Use embed-english-v3.0 with search_query input type for queries
    ...
    def retrieve(self, query_text: str, top_k: int = 5, threshold: float = 0.0, include_metadata: bool = True) -> str:
        ...
        if include_metadata:
            self.verify_content_accuracy(raw_results)
